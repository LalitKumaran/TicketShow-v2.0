              ...
              >
                <table class="table table-dark">
                  ...
                  <tbody>
                    ...
                    <tr>
                      <th>Theatre</th>
                      <td>{{ selectedTheatre.venue_name }}</td>
                    </tr>
                    <tr>
                      <th>Location</th>
                      <td>{{ selectedTheatre.location }}</td>
                    </tr>
                    <tr>
                      <th>Seats</th>
                      <td>{{ seatCount }}</td>
...
export default {
  ...
  methods: {
    confirmBooking() {
      ...
      axios
        .post(
          "http://127.0.0.1:5000/api/bookshow",
          {
        ...
        for (const theatre of this.theatres) {
          if (theatre.venue_id == this.selectedSlot.venue_id) {
            ...
            if (this.selectedMovie.rating < 6) {
              this.amount = (theatre.price - 20) * this.seatCount;
            } else if (this.selectedMovie.rating > 6) {
              this.amount = (theatre.price + 20) * this.seatCount;
            } else {
              this.amount = theatre.price * this.seatCount;
            }
            break;
          }
        }
